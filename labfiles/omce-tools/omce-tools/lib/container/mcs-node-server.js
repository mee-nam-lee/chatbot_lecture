...
exports.start = function(offlineContext) {
    ...
    // 8.9.4 -> 8
    var nodeMainVersionInt = parseInt(process.versions.node.split('.')[0]);
    ...
    console.is_finest = console.is_finer = console.is_fine = console.is_config = console.is_info = console.is_warning = console.is_severe = function(){
        return true;
    };
    ...
    if (offlineContext.proxy){
        if (offlineContext.proxy.http){
            ...
            var httpProxyOptions = 'http://' + offlineContext.proxy.http.host + ':' + offlineContext.proxy.http.port;
        ...
        if (offlineContext.proxy.https){
            ...
            var httpsProxyOptions = 'http://' + offlineContext.proxy.https.host + ':' + offlineContext.proxy.https.port;
    ...
    var server = service.listen(port, function() {
