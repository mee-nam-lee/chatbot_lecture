		...
		getCollection: function(collection, opts, httpOptions){
			...
			return utils().wrap(rest, 'get', options, opts);
                ...
                doesCollectionExist: function(collection, opts, httpOptions){
                        var optionsList = buildOptionsList(buildBaseCollectionURI(collection), opts, httpOptions);
                        return utils().wrapResult(rest, 'head', optionsList).then(
                                function(result){
                                            if (result.statusCode == 200){
/*                                                    if (console.is_finest()) {
                                                        console.finest('OracleMobile.storage Promise response -> result: true');
                                                    }*/
                                                    return Promise.resolve({result: true, statusCode: 200})
                                            }
/*                                            if (console.is_finest()) {
                                                console.finest('OracleMobile.storage Promise response -> result: false');
                                            }*/
