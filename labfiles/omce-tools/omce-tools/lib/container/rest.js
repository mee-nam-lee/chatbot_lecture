...
function rest(internalParams) {
    ...
    var restInstance = Object.create({
		...
		patch: function(optionsList,handler){
			return buildRequest('patch', cloneOptions(optionsList), handler);
		},

		patchAsync: function(optionsList){
			return buildRequestAsync('patch', cloneOptions(optionsList));
		},
	...
	function requestPlusAsync(callOptions, options) {
		...
		switch(callOptions.method){
			...
			case 'post':
				...
				break;
			case 'patch':
				return request.patchAsync(updatedOptions);
			break;
			case 'options':
